// src/pages/MateriSubtopicListPage.jsx
    ...
    const fetchSubtopics = async () => {
      try {
        const { data, error } = await supabase
          ...
          .eq("topic_id", topicId)
          .order("sort_order", { foreignTable: "subtopics", ascending: true })
          .single();
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <p className="text-red-500">Gagal memuat subtopik materi: {error}</p>
        <button
          onClick={() => navigate(-1)}
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap items-center gap-2 text-gray-500 mb-4 text-sm">
        ...
        <Link
          to={`/materi/${data.category.category_id}`}
        ...
        {subtopics.map((subtopic) => (
          <div key={subtopic.id}>
            {subtopic.is_published ? (
              <Link
                // Link ke MateriListPage (daftar materi dalam subtopik)
                to={`/materi/${categoryId}/${topicId}/${subtopic.subtopic_id}`}
...
export default MateriSubtopicListPage;
